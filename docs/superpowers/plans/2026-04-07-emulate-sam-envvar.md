...
**Architecture:** `EMULATE_SAM` env var activates in-process SAM emulation. `CarBus::SAM` gets a new domain method (`set_zone_setpoint`) that composes existing state/bus methods. The infinitude app hooks into the existing setpoint API handler to route writes through RS485 with XML fallback. Unknown register reads return exception frames instead of silence.
...
|------|--------|----------------|
| `lib/CarBus/SAM.pm` | Modify | Add `_exception_reply`, `set_zone_setpoint` domain method |
| `infinitude` | Modify | Read env var, init SAM, intercept frames in websocket loop, hook setpoint API |
| `t/sam-emulator.t` | Modify | Add tests for exception replies and `set_zone_setpoint` |
| `t/carbus-develop.t` | Modify | Update test that expects `undef` for unknown register reads |

---

### Task 1: Add `_exception_reply` and fix `_handle_read`

Unknown register reads currently return `undef` (silently dropped). They must return an exception frame with code `0x04`.

**Files:**
- Modify: `lib/CarBus/SAM.pm:381-401`
...
```perl
    ...
    is($exc_reply->struct->{src}, 'FakeSAM', 'exception reply src is FakeSAM');
...
In `lib/CarBus/SAM.pm`, add the `_exception_reply` method after `_handle_write` (after the closing `}` of `_handle_write`, around line 427):
...
Then replace `_handle_read` (lines 382-401) with:
...
### Task 2: Add `set_zone_setpoint` domain method to CarBus::SAM
...
```perl
# Test 11: set_zone_setpoint domain method
...
Add to `lib/CarBus/SAM.pm` after the existing `write_thermostat` method (around line 438):
...
sub set_zone_setpoint {
    ...
    # Write to thermostat on bus
    my ($table, $row) = (0x3B, 0x03);
    $self->bus->write_register('Thermostat', $table, $row, $new_data);
...
### Task 3: Wire `EMULATE_SAM` into infinitude

Add env var, SAM initialization, and frame interception in the websocket loop.
...
### Task 4: Hook setpoint API to route through RS485
...
### Task 5: Update CLAUDE.md with new configuration
